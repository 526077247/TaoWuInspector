/** Runtime 脚本模板 */
const RUNTIME_TEMPLATES: { fileName: string; content: string }[] = [
    {
...
export class ExampleImporter {
     ...
     */
    static async installRuntime(): Promise<void> {
     ...
     */
    static async importExample(): Promise<void> {
     ...
     */
    private static async openSceneAndAddNode(sceneUuid: string): Promise<void> {
        try {
    ...
    /** 确保目录存在 */
    private static async ensureDir(url: string): Promise<void> {
    ...
    /** 等待文件入库 + 编译 */
    private static async waitFileReady(url: string): Promise<void> {
        for (let i = 0; i < 30; i++) {
    ...
    /** 构建场景 JSON */
    private static buildSceneJson(): string {
