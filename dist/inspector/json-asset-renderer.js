...
exports.jsonState = exports.$ = exports.template = void 0;
...
function update(assetList, metaList) {
    ...
    exports.jsonState.uuid = asset.uuid;
    exports.jsonState.url = asset.url || '';
    exports.jsonState.onDirty = updateDirtyUI;
    Editor.Message.request('taowu-inspector', 'read-asset-file', asset.uuid)
        ...
        try {
            ...
            const cleanText = text
                ...
                .replace(/(^|[^:])\/\/[^\n\r]*/g, '$1')
                .replace(/,\s*([}\]])/g, '$1');
            exports.jsonState.asset = JSON.parse(cleanText);
        }
        catch (e) {
            content.innerHTML = '<div class="json-asset-empty">Invalid JSON: ' + e.message + '</div>';
            return;
        }
        // 存储 rerender 回调，供 property-drawer 在属性变更后触发重新渲染 (ShowIf/HideIf 等)
        content.__taowuRerender = (changedProp, newVal) => {
            if (changedProp !== undefined && exports.jsonState.asset) {
                exports.jsonState.asset[changedProp] = newVal;
            }
            // 检查 ShowIf/HideIf 和 EnableIf/DisableIf 条件是否变化
            const _meta = exports.jsonState.classMeta || {};
            const _props = {};
            if (exports.jsonState.asset && typeof exports.jsonState.asset === 'object') {
                for (const k of Object.keys(exports.jsonState.asset)) {
                    if (k !== '_t')
                        _props[k] = exports.jsonState.asset[k];
                }
            }
            ...
            for (const key of Object.keys(_meta)) {
                ...
                if (m?.showIf || m?.hideIf) {
                    if ((0, taowu_utils_1.evaluateCondition)(m, condProps))
                        visibleKeys += 's' + key + ',';
                }
                if (m?.enableIf || m?.disableIf) {
                    if ((0, taowu_utils_1.evaluateEnabled)(m, condProps))
                        visibleKeys += 'e' + key + ',';
                }
            }
            if (visibleKeys !== exports.jsonState.lastVisibleKeys) {
                exports.jsonState.lastVisibleKeys = visibleKeys;
                renderJsonAsInspector(content, exports.jsonState.asset);
            }
            // 条件没变化时不重新渲染，避免滑块断触
        };
        renderJsonAsInspector(content, exports.jsonState.asset);
...
function close() {
    if (exports.jsonState.dirty && exports.jsonState.asset && exports.jsonState.url) {
        const savedAsset = exports.jsonState.asset;
        const savedUrl = exports.jsonState.url;
    ...
    exports.jsonState.url = '';
    exports.jsonState.dirty = false;
    exports.jsonState.onDirty = null;
}
function updateDirtyUI() {
    const dirtyEl = document.querySelector('.json-dirty-indicator');
    const saveBtn = document.querySelector('.json-save-btn');
    if (dirtyEl)
        dirtyEl.style.display = exports.jsonState.dirty ? 'inline' : 'none';
    if (saveBtn) {
        if (exports.jsonState.dirty)
...
async function saveJson() {
    if (!exports.jsonState.asset || !exports.jsonState.url)
        return;
    const saveBtn = document.querySelector('.json-save-btn');
    if (saveBtn) {
    ...
    try {
        // 直接用 JSON.stringify 保留所有字段 (包括 _t)
        const content = JSON.stringify(exports.jsonState.asset);
        const result = await Editor.Message.request('asset-db', 'save-asset', exports.jsonState.url, content);
        if (result) {
            exports.jsonState.dirty = false;
            updateDirtyUI();
...
function saveJsonSync() {
    if (!exports.jsonState.asset || !exports.jsonState.url)
        return;
    try {
        const content = json_helper_1.JsonHelper.toJson(exports.jsonState.asset, false);
        Editor.Message.request('asset-db', 'save-asset', exports.jsonState.url, content);
    }
    catch (e) {
...
function jsonValueToDump(val, key) {
    if (val === null) {
...
async function renderJsonAsInspector(container, json) {
    if (typeof json !== 'object' || json === null) {
        const dump = { value: json, type: typeof json === 'number' ? 'Number' : typeof json === 'boolean' ? 'Boolean' : 'String', name: 'value' };
        const el = (0, property_drawer_1.createPropertyElement)('value', dump, 'json-edit', 0, {}, () => false, () => { });
        container.appendChild(el);
        return;
    }
    const typeName = json._t ? String(json._t) : '';
    exports.jsonState.rootTypeName = typeName;
    let classMeta = {};
    ...
    if (typeName) {
        try {
            ...
            classMeta = meta || {};
            exports.jsonState.classMeta = classMeta;
        }
        catch (e) {
            classMeta = {};
            exports.jsonState.classMeta = classMeta;
    ...
    const queriedTypes = new Set();
    const collectNestedTypes = (obj) => {
        if (!obj || typeof obj !== 'object')
            return;
        if (Array.isArray(obj)) {
            obj.forEach(collectNestedTypes);
            return;
        }
    ...
    for (const nt of queriedTypes) {
        if (nt.startsWith('cc.') || nt === 'Number' || nt === 'String' || nt === 'Boolean' || nt === 'Object')
            continue;
        try {
...
function renderWithMeta(container, json, classMeta, elementMetadata = {}, propertyTypes = null) {
    ...
    for (const key of allKeys) {
        ...
        if (val && typeof val === 'object' && !Array.isArray(val)) {
            if ('x' in val && 'y' in val && 'z' in val && !('w' in val))
                cocosType = 'cc.Vec3';
            else if ('x' in val && 'y' in val && 'z' in val && 'w' in val)
                cocosType = 'cc.Vec4';
            else if ('x' in val && 'y' in val && !('z' in val))
                cocosType = 'cc.Vec2';
            else if ('r' in val && 'g' in val && 'b' in val)
                cocosType = 'cc.Color';
            else if ('width' in val && 'height' in val)
                cocosType = 'cc.Size';
        }
    ...
    for (const [groupPath, fkeys] of organized.foldoutGroups) {
        const el = (0, group_drawer_1.createFoldoutGroup)(groupPath, fkeys, dump, 'json-edit', 0, classMeta, () => false, () => { }, elementMetadata);
        container.appendChild(el);
    }
    // 3. Tab 分组
    for (const [groupName, tabs] of organized.tabGroups) {
        const el = (0, group_drawer_1.createTabGroup)(groupName, tabs, dump, 'json-edit', 0, classMeta, () => false, () => { }, elementMetadata);
        container.appendChild(el);
    }
    // 4. Box 分组
    for (const [groupName, bkeys] of organized.boxGroups) {
        const el = (0, group_drawer_1.createBoxGroup)(groupName, bkeys, dump, 'json-edit', 0, classMeta, () => false, () => { }, elementMetadata);
        container.appendChild(el);
    }
    // 5. Horizontal 分组
    for (const [groupName, hkeys] of organized.horizontalGroups) {
        const el = (0, group_drawer_1.createHorizontalGroup)(groupName, hkeys, dump, 'json-edit', 0, classMeta, () => false, () => { }, elementMetadata);
        container.appendChild(el);
    }
}
/** 根据元数据推断默认值 */
function inferDefaultValue(meta) {
    if (meta?.range)
        return meta.range.min;
    if (meta?.valueDropdown) {
        const vd = meta.valueDropdown;
        if (vd.values && vd.values.length > 0)
            return vd.values[0];
        return 0;
    }
    if (meta?.tableList)
...
/** 根据元数据和默认值推断类型 */
function inferDefaultType(meta, defaultVal) {
    if (meta?.valueDropdown) {
        return typeof defaultVal === 'number' ? 'Number' : 'String';
    }
    if (meta?.tableList || Array.isArray(defaultVal))
        return undefined;
