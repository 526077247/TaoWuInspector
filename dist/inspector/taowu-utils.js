...
/** 从 dump 数据中提取组件 UUID */
function getComponentUuid(dump) {
    if (dump.uuid) {
        if (typeof dump.uuid === 'object')
            return dump.uuid.value || '';
        return dump.uuid;
    }
    if (dump.value?.uuid?.value)
        return dump.value.uuid.value;
    return '';
}
/**
 * 从 dump 数据中提取组件类型名
 ...
 */
function getComponentType(dump) {
    // 方式1: dump 顶层 __type__
    if (dump.__type__)
        return dump.__type__;
    // 方式2: dump.value.type.value
    if (dump.value?.type?.value)
        return dump.value.type.value;
...
/** 评估 ShowIf/HideIf 条件 */
function evaluateCondition(meta, properties) {
    if (!meta)
        return true;
    if (meta.showIf) {
...
function organizeProperties(propertyKeys, taowuMeta) {
    const result = {
        ungrouped: [],
        foldoutGroups: new Map(),
        tabGroups: new Map(),
        boxGroups: new Map(),
        horizontalGroups: new Map(),
    };
